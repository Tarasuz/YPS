...
export const sideBarEnteredAnimation = (
  sidebarPosition: number,
  prevSidebarPosition?: number
) => keyframes`

    0% {
      left: ${prevSidebarPosition}px;
    }
...
export const Wrapper = styled.div<WrapperProps>`
  ...
  animation: ${p => {
    switch (p.animationState) {
      case "entered":
        return sideBarEnteredAnimation(
          p.currentSideBarPosition,
          p.prevSidebarPosition
        );
      case "entering":
...
export const SidebarOverlay = styled.div<OverlayProps>`
  position: fixed;
  top: 0;
  right: 0;
  bottom: 0;
